...
/**
 * **適用範圍** —— 篩查涵蓋得到的實足月齡區間，也就是畫面上的「12-180 个月（1-15岁）」。
 *
 * 由年齡段反推，不另外寫死一組數字：這兩個數字原本散在兩張表單的驗證式與說明文字裡，
 * 與這裡的年齡段各自為政。哪天加一段（例如往下開到 6 個月），漏改的那一邊會表現成
 * 「選得到、但選了就被擋下」或反過來「選不到、卻其實有題目」。
 */
export const T1_AGE_RANGE = {
  minMonths: T1_AGE_BANDS[0].minAge,
  maxMonths: T1_AGE_BANDS[T1_AGE_BANDS.length - 1].maxAge,
} as const;
