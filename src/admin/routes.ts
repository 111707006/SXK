...
export function createAdminRouter(shape: AdminCenterShape, hooks: AdminRouterHooks = {}): express.Router {
  ...
  // ── 活動庫標記頁（#62，規格 v2 §7.4）──
  //
  // 活動不是家長資料，因此這兩支**不經過 `withScope`** —— 沒有一位家長的資料會從
  // 這裡出去，硬套一個公司條件只會讓人以為活動分公司。但它們仍然限定全域管理員：
  // 活動是森心康的干預內容，合作公司不維護它，而專案 B 根本沒有深度評估。少了
  // `requireGlobal`，一家合作公司就能改掉所有孩子每週拿到的訓練。
  //
