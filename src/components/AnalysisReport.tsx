...
interface AnalysisReportProps {
  ...
  /**
   * 即時報告接續上一次生成的那一份（只在 `historicalRecord` 為 null 時看）。
   *
   * 生成的報告本來只活在這個元件的 state 裡：去一趟付費牆再回來，元件重掛，
   * 家長看到的又是「一键生成」—— 付完錢先得再等一次 AI、再找一次 T2 入口。
   * 生成時已經歸檔一份（`onSaveReportToHistory`），App 從歷史裡挑出**分數與現在
   * 完全相同**的最新一筆傳進來，這裡直接接著顯示；分數變了就不會傳，照舊從頭生成。
   */
  resumeFrom?: AssessmentRecord | null;
  /**
   * 一進來就生成（面板上「生成全维 AI 深度评估报告」那顆按鈕，2026-09-16）。
   * 按鈕的字是「生成」，跳到這頁再要家長按一次「一键生成」是多一步。
   * 已經有報告（`resumeFrom` 接上、或歷史紀錄）就不再打一次 —— 只在什麼都沒有時才生成。
   */
  generateOnOpen?: boolean;
  /**
   * 一進來就捲到 T2 入口那張卡（付費／跳過付費之後回來，2026-09-16）。
...
export default function AnalysisReport({ child, completedScores, onBack, onSaveReportToHistory, onGoToLanguageSpecial, t2, historicalRecord, focusBooking, focusBookingService, resumeFrom, generateOnOpen, focusT2 }: AnalysisReportProps) {
  ...
  const [isAiGenerated, setIsAiGenerated] = useState<boolean | null>(() => (resumed ? (resumed.isAiGenerated ?? null) : false));
  ...
  const t2SlotRef = useRef<HTMLDivElement>(null);

  // 付費回來：報告在（接續上一份）、入口卡在，就捲到那張卡。只捲一次。
  const t2AutoScrolledRef = useRef(false);
  useEffect(() => {
    if (!focusT2) { t2AutoScrolledRef.current = false; return; }
    if (t2AutoScrolledRef.current || !aiReport) return;
    // 報告底下的圖表是掛上去之後才長高的，捲一次會被後面的版面位移推回去；
    // 捲兩次（畫完那一輪、圖表長完之後），用瞬間捲不用平滑 —— 平滑捲會被位移打斷。
    // 「捲過了」在真的捲到時才記：StrictMode 會把 effect 跑兩次，第一次的計時器被清掉，
    // 先記的話第二次就以為捲過了。
    const scroll = () => {
      t2SlotRef.current?.scrollIntoView({ behavior: 'auto', block: 'start' });
      t2AutoScrolledRef.current = true;
    };
    const timers = [setTimeout(scroll, 250), setTimeout(scroll, 1200)];
  ...
  // 面板按鈕帶著「到了就生成」進來：沒有任何報告可顯示才打，有就直接顯示（見 prop 說明）。
  const autoGenerated = useRef(false);
  useEffect(() => {
    if (!generateOnOpen || autoGenerated.current) return;
    if (historicalRecord || aiReport || loading) return;
