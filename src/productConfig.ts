...
export interface ProductProfile {
   ...
   * 建置模式標記 —— 說明當前是哪一個產品。
   *
   * ⚠️ **2026-09-10 起不再顯示在畫面上。** 它是開發與驗收要的訊號，對家長沒有
   * 意義，而 B 交付給合作公司，掛一個看不懂的內部標籤只會招來「T1 是什麼」。
   * 現在渲染成一個 `hidden` 的節點（見 `App.tsx` 的說明）—— 版面上沒有它，
   * 但「這份產物是 A 還是 B」仍然 grep 得到，那是每次部署最後一道檢查。
   *
   * `className` 因此暫時沒有呼叫端。留著不刪：要把徽章調回可見只是拿掉 `hidden`，
   * 而重新配一次色不是零成本的事。
   */
  buildBadge: {
    /** 徽章上的字，越短越好 */
