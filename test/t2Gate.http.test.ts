 ...
 *
 * 測試只斷言**它不是一次拒絕**（#57 補上處理函式之後，放行是一個 200 的空清單；
 * 補上之前是落到 /api 的 JSON 404 —— 兩種都不是拒絕，斷言的意思沒變）。
 */
...
// #57 之後閘門後面有處理函式了：清單給空的，讓「放行」是一個 200，而不是替身缺函式的 500。
vi.mock('../src/db/t2ToolResults', () => ({
  insertToolResult: async () => 1,
  listToolResults: async () => [],
}));

/** 閘門是路徑前綴的守衛，一條 T2 路徑就夠；處理函式本身由 `t2ToolResults.http.test.ts` 驗。 */
