/**
 * 家長掃碼帶走的那一頁（`/r/:token`）。伺服器拼字串產生的純 HTML。
 *
 * 【它現在只有一個呼叫端】
 * 2026-09-11 之前，後台的匯出（`GET /admin/parents/:id/export`）也走這裡。那條路
 * 隨 ADR-0007 移除 —— 後台改成顯示家長看到的那份報告，而這裡畫不出雷達圖、儀表
 * 與軌跡圖（沒有 React、沒有 Tailwind）。要讓它長得一樣，等於用字串再實作一次
 * 整份報告，之後每次改報告都要改兩處。
 *
 * 掃碼那一頁**刻意留在這裡**：它的價值是「不靠 JS 也打得開」，改成 React 頁面
 * 就失去了。代價是它與家長在手機上看到的那一頁長得不一樣，這是 ADR-0007 明說
 * 的已知差異，不是待辦。
 *
 * 【它不碰資料庫】
 * 只接受一份已經取好的資料 —— 自己再查一次就會成為繞過公司範圍的第二條路。
 ...
 * 這一頁真正用得到的欄位。
 *
 * 家長掃碼帶走報告的那條路（issue #22）走的不是後台的資料入口，它拿不到
 * `companyId`、`hasBooking` 這些後台專用的欄位。宣告成這個較小的型別，是為了讓
 * 「這一頁需要什麼」看得出來，而不是要求呼叫端捏造幾個用不到的值。
 *
 * `ParentDetail` 是它的超集，所以拿一份後台詳情直接傳進來也成立（測試就是這樣做的）。
 */
...
export interface ReportRenderOptions {
  ...
  reportId?: string;
  /**
   * 要不要列出專家預約。**唯一的呼叫端（家長的永久連結）刻意關掉它**：預約區塊
   * 裡有聯絡人姓名與手機號，而這條連結撤不回來。報告本身已經是敏感資料，沒有
   * 理由讓它再多帶一組聯絡方式出門。
   *
   * 預設仍是「要」—— 那是這個參數加進來之前的行為，測試靠它驗證預設值沒有翻面。
   */
   ...
   * 寫死一個佔位號碼比沒有更糟）。
   *
   * **由呼叫端傳入，這個檔案不讀 `process.env`** —— 家長掃碼打開的那一頁是網站
   * 的一頁，備案號必須在；而把它列印出來的那份文件不是網頁，掛備案號沒有意義。
   * 差別寫在呼叫端才看得見。
   */
  icpBeian?: string;
}
